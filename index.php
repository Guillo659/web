                ...
                if ($row["publictype"] === 's') {
                    ...
                    <article class="article" data-article-id="<?= $row["id"] ?>">
                        <section class="art-info">
                            ...
                            <div class="art-text">
                                <a role="h1" href="view/view.php?id=<?= $row["id"] ?>"><?= $row["title"] ?></a>
                                <a role="p" tabindex="-1" href="view/view.php?id=<?= $row["id"] ?>"><?= $sub ?></a>
                            </div>
                            <div class="art-ctg">
                                <?php 
            ...
            if ($row["publictype"] === 'p') {
                ...
                <article class="post" data-post-id="<?= $row['id'] ?>">
                    <section class="head-user" data-user-id="<?= $row["user_id"] ?>">
                        <img src="<?= $row["profileimage"] ?>" alt="Profile image">
                        <div class="user-date">
                            <a href="view/profile.php?id=<?= $row["user_id"] ?>"><?= $row["name"] ?></a>
                            <span title="<?= date('D · g:i A', strtotime($row['date_created']))?>"><?= $day, date($formato_fecha, strtotime($row['date_created'])) ?></span>
                        </div>
                        <button class="btn-more" aria-label="Options">
                            <span aria-hidden="true" class="material-symbols-rounded">more_horiz</span>
        ...
        xhr.onload = () => {
            if (xhr.status === 200) {
                ...
                comments.forEach(comment => {
                    ...
                    if (!lastDay) {
                        const monthName = dateComment.toLocaleDateString('en-US', {month: 'short'})
                        fecha = `${monthName} ${day}, ${year}`
                    }

                    const weekDay = dateComment.toLocaleDateString('en-US', {weekday: 'short'})
